...
public class FornitoreDao {
    ...
    public List<Fornitore> findAll() { return findAll(false); }
    ...
    public void insert(Fornitore f) {
        ...
        try (Connection c = Database.getConnection();
            ...
            if (msg.contains("unique") && (msg.contains("fornitori.codice_fornitore") || msg.contains("ux_fornitori_codice_fornitore"))) {
                throw new DuplicateKeyException("Codice fornitore già esistente: " + f.getCodiceFornitore(), ex);
            }
    ...
    public void update(Fornitore f) {
        ...
        String sql =
            ...
            "attivo=?, ragione_sociale=?, indirizzo=?, cap=?, citta=?, provincia_id=?, nazione_id=?, " +
            "telefono=?, cellulare=?, piva=?, cf=?, iva_id=?, pagamento_id=?, email=?, note=? " +
            "WHERE id=?";
        ...
        try (Connection c = Database.getConnection();
            ...
            ps.setInt(1, f.isAttivo() ? 1 : 0);
            ps.setString(2, f.getRagioneSociale());
            ps.setString(3, f.getIndirizzo());
            ps.setString(4, f.getCap());
            ps.setString(5, f.getCitta());

            if (f.getProvinciaId() != null) ps.setLong(6, f.getProvinciaId()); else ps.setNull(6, java.sql.Types.INTEGER);
            if (f.getNazioneId() != null) ps.setLong(7, f.getNazioneId()); else ps.setNull(7, java.sql.Types.INTEGER);

            ps.setString(8, f.getTelefono());
            ps.setString(9, f.getCellulare());
            ps.setString(10, f.getPiva());
            ps.setString(11, f.getCf());

            if (f.getIvaId() != null) ps.setLong(12, f.getIvaId()); else ps.setNull(12, java.sql.Types.INTEGER);
            if (f.getPagamentoId() != null) ps.setLong(13, f.getPagamentoId()); else ps.setNull(13, java.sql.Types.INTEGER);

            ps.setString(14, f.getEmail());
            ps.setString(15, f.getNote());

            ps.setLong(16, f.getId());
