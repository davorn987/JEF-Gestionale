...
import it.jeansandfriends.gestionale.ui.clienti.ClienteEditDialog;
import it.jeansandfriends.gestionale.ui.clienti.ClientiTableModel;

public class ClientiPanel extends JPanel {

    private final ClienteDao dao = new ClienteDao();
    private final ClientiTableModel model = new ClientiTableModel();
    private final JTable table = new JTable(model);

    private final TableRowSorter<ClientiTableModel> sorter = new TableRowSorter<>(model);
    ...
    private void refresh() {
        List<Cliente> all = dao.findAll(chkIncludeInactive.isSelected());
        model.setRows(all);
        applyFilter();
    }
    ...
    private void applyFilter() {
        ...
        sorter.setRowFilter(new RowFilter<ClientiTableModel, Integer>() {
            @Override
            public boolean include(Entry<? extends ClientiTableModel, ? extends Integer> entry) {
    ...
    private void onNew() {
        ...
        int nextCode = dao.getNextCodiceCliente();

        ClienteEditDialog dlg = new ClienteEditDialog(owner, "Nuovo Cliente", null, nextCode);
        dlg.setVisible(true);

        if (dlg.isConfirmed()) {
            try {
                dao.insert(dlg.getCliente());
                refresh();
    ...
    private void onEdit() {
        Cliente selected = getSelected();
        if (selected == null) return;

        JFrame owner = (JFrame) javax.swing.SwingUtilities.getWindowAncestor(this);
        ClienteEditDialog dlg = new ClienteEditDialog(owner, "Modifica Cliente", selected, 0);
        dlg.setVisible(true);

        if (dlg.isConfirmed()) {
            try {
                dao.update(dlg.getCliente());
                refresh();
            } catch (RuntimeException ex) {
                JOptionPane.showMessageDialog(this, ex.getMessage(), "Errore", JOptionPane.ERROR_MESSAGE);
            }
    ...
    private void onDisable() {
        ...
        int confirm = JOptionPane.showConfirmDialog(this,
                "Oscurare il cliente " + String.format("%06d", selected.getCodiceCliente()) + " ?",
                "Oscura Cliente",
    ...
    private Cliente getSelected() {
        ...
        if (viewRow < 0) {
            JOptionPane.showMessageDialog(this, "Seleziona un cliente.", "Info", JOptionPane.INFORMATION_MESSAGE);
            return null;
        }
